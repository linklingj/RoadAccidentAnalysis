...
U-Net 출력을 기존 `infer_road_model()` 인터페이스(`road_polygons_uv`)에 맞췄기 때문에 **도로 단계만**
토글로 교체된다. 객체 단계의 `object_detector_type` 패턴과 동일하게 `PipelineConfig.road_detector_type`
(`"yolo"` | `"unet"`) 플래그로 제어한다. U-Net 의존성(`segmentation_models_pytorch`)은 `"unet"` 일
때만 **지연 import** 되므로 YOLO 사용 환경은 영향받지 않는다.

**관련 config** (`infer.py::PipelineConfig`):
```python
road_detector_type: str = "yolo"          # "yolo" | "unet"
road_unet_model_path: str = "runs/smp-road/best.pt"
road_unet_imgsz: int = 512
road_unet_threshold: float = 0.5
road_unet_min_area: int = 1500            # 작은 노이즈 도로 조각 폐기
```

**구현 지점**:
- `RoadSceneProjector.__init__`: 플래그에 따라 YOLO 또는 U-Net 체크포인트 로드.
- `RoadSceneProjector._infer_road(image_bgr)`: 백엔드 추상화. `run()`·`run_video()` 의 도로 추론 호출이
  이 메서드로 통일됨 → 이후 BEV 투영·`filter_outside_road`·scene export 전부 **무수정**.
...
# U-Net 도로 백엔드로 토글
python main.py --image input/image1.png --use-unet-road
python main.py --video input/video1.mp4 --use-unet-road --road-unet-model runs/smp-road/best.pt
```

**사용** (코드):
```python
from infer import PipelineConfig, run_pipeline
cfg = PipelineConfig(road_detector_type="unet")   # 나머지 파이프라인 동일
run_pipeline(image_path="input/image1.png", save_dir="output", config=cfg)
```

> 참고: U-Net 은 시맨틱 마스크를 connected-component 폴리곤으로 변환하므로 도로가 여러 조각으로 나뉠 수
> 있다(YOLO 의 단일 병합 폴리곤과 달리). 모두 유효한 도로 영역이며 `road_unet_min_area` 로 노이즈 조각만
> 걸러낸다. 후속 BEV/필터 단계는 다중 폴리곤을 이미 정상 처리한다.
