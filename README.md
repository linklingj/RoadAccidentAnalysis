...
> **2D CCTV 영상 한 개** 만으로 도로·차량·궤적을 추출하고,
> BEV(Bird's-Eye-View) 월드 좌표(m) 로 변환해 **브라우저 three.js/WebGL 뷰어**에서
> 교통사고 현장을 **3차원으로 재구성하는** 딥러닝 파이프라인.

<p align="center">
  <a href="http://roadaccident-alb-1568059849.ap-northeast-2.elb.amazonaws.com/">
    <img src="https://img.shields.io/badge/🚀%20Live%20Demo-바로%20체험하기-2EA043?style=for-the-badge&logoColor=white" alt="Live Demo" height="30">
  </a>
</p>

<p align="center">
  <b>▶ 라이브 데모:</b>
  <a href="http://roadaccident-alb-1568059849.ap-northeast-2.elb.amazonaws.com/">roadaccident-alb-1568059849.ap-northeast-2.elb.amazonaws.com</a>
  <br>
  <sub>AWS ECS Fargate 배포 · 영상/이미지를 업로드하면 서버가 추론해 3D로 재구성한다 (비용 절감을 위해 미가동 시간이 있을 수 있음)</sub>
</p>
...
![demo](docs/assets/webpage/sample1.png)
...
> 🌐 **라이브 데모**: **<http://roadaccident-alb-1568059849.ap-northeast-2.elb.amazonaws.com/>**
> (AWS ECS Fargate 배포)
...
![demo](docs/assets/webpage/demo.gif)

<p align="center"><sub>업로드 → 추론 → 3D 재구성 전체 흐름</sub></p>
...
![sample](docs/assets/webpage/bev.png)
![sample](docs/assets/webpage/output.png)

> 🎞️ **데모 영상 (텍스트 노트)**: `output/sample1_tracked_bev.mp4`, `output/sample2_tracked_bev.mp4`,
> `output/sample3_tracked_bev.mp4` 에 BEV 추적 영상이 포함되어 있다.
...
| 사고 테스트셋 | [Kaggle](https://www.kaggle.com/datasets/webadvisor/real-time-anomaly-detection-in-cctv-surveillance) · [CarCrash](https://ankitshah009.github.io/accident_forecasting_traffic_camera) | 추론 정성 평가 | 영상 약 30개 |
...
| 도로 분할 — YOLO26-seg `0401` | roadseg val | mask mAP50 / mAP50-95 | 0.985 / 0.980 |
| 횡단보도 — YOLO26-seg `0407` | crosswalk val | box mAP50 / mask mAP50 | 0.974 / 0.871 |
| 객체 탐지 — YOLO26-seg `0619` | object val | mask mAP50 / mAP50-95 | 0.763 / 0.475 |
| 객체 탐지 — **RF-DETR Nano (augmented)** | held-out test 333장 | mAP50 / mAP50-95 | 0.591 / 0.367 |

> 도로·횡단보도는 대면적·정형 클래스라 지표가 높고, 객체 탐지는 소형·원거리·야간 차량/보행자를
> 포함해 난도가 높다. RF-DETR 증강 실험의 상세 분석은 [§8.2](#82-객체-탐지-ai-hub-데이터-증강).

### 7.2 학습 곡선

**U-Net 도로 분할** — 1 epoch 만에 val IoU 0.92, 0.97로 수렴:

![U-Net training curve](docs/assets/unet_vs_yolo/training_curve.png)

**YOLO26-seg 객체 탐지** — box/mask loss 감소와 mAP 상승 (train/val 23개 지표):

![object training curves](docs/assets/training/object_results.png)

### 7.3 데이터 증강 (학습 배치)

학습 시 적용되는 CCTV 특화 augmentation을 실제 학습 배치에서 확인 — 모자이크·조도 변화·미세 회전이
적용된 도로 마스크(파란 오버레이):

![road train batch](docs/assets/training/road_train_batch.jpg)

### 7.4 검증 정성 결과

**객체 탐지 — 정규화 혼동행렬** (대각이 정탐). car 0.90 · bus 0.86 · truck 0.76으로 강하고,
소형·원거리 person(0.44)·riders(0.65)은 background와 혼동되는 경향:

![object confusion matrix](docs/assets/training/object_confusion_matrix.png)

**객체 탐지 검증 예측** — 야간 교차로 차량/라이더 탐지 (좌: 객체 · 우: 횡단보도 인스턴스 분할):

<p align="center">
  <img src="docs/assets/training/object_val_pred.jpg" width="48%">
  <img src="docs/assets/training/crosswalk_val_pred.jpg" width="48%">
</p>
...
### 10.3 정적 뷰어 (영상 업로드 불가, 샘플만 이용 가능)
